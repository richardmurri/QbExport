    ...
    def get_data_row(self, line, columnArray):
        ...
        if line[0]:
            ...
            if 'Credit' in columnArray and 'Debit' in columnArray:
                ...
                debit = self.get_decimal(new['Debit']) * -1
...
class IIFGenerator(object):
    file_start_tpl = '!TRNS\tTRNSID\tTRNSTYPE\tDATE\tACCNT\tNAME\tAMOUNT\tDOCNUM\tMEMO\tCLEAR\tTOPRINT\tADDR1\tADDR2\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\n' \
                     ...
                     '!ENDTRNS\n'
    trans_tpl = 'TRNS\t\t%s\t%s\t%s\t%s\t%s\t\t%s\t%s\t%s\t%s\t%s\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\n'
    split_tpl = 'SPL\t\t%s\t%s\t%s\t%s\t%s\t\t%s\t%s\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\n'
    ...
    def write_transaction(self, trans, splits, f):
        ...
        trans = self.trans_tpl % (tType, date, accountName, name, amount, memo, cleared, toPrint, address1, address2)
        f.write(trans)
        ...
        for spl in splits:
            ...
            splCleared = 'N'
            splString = self.split_tpl % (tType, date, splAccount, name, splAmount, memo, splCleared)
            f.write(splString)
